...
- ✅ `Dockerfile` (Docker configuration with Tesseract)
- ✅ `requirements.txt` (Python dependencies)
- ✅ `app.py` (Flask application)
- ✅ All service files and routes
...
- Render will automatically detect `Dockerfile` in your Backend folder
- Select "Backend" as the root directory
- Render will build the Docker image automatically
- No need to build locally - Render handles everything!

**Manual Configuration (if not using Dockerfile)**
...
- Dockerfile installs Tesseract automatically during build
- Check if `TESSERACT_CMD=/usr/bin/tesseract` environment variable is set
